...
WsEventMgr = function (options) {
  ...
  this.options.registerEventName = this.options.registerEventName || 'register';
  this.options.unregisterEventName = this.options.unregisterEventName || 'unregister';
...
WsEventMgr.prototype.start = function (server, connectCallback, disconnectCallback) {
  ...
  that.server.on('connection', function (socket) {
    ...
    socket.on(that.options.registerEventName, function (event) {
      ...
      that.clientRegistrations[event].push(socket.id);
    ...
    socket.on(that.options.unregisterEventName, function (event) {
      var index;
      index = that.clientRegistrations[event].indexOf(socket.id);
      if (index !== -1) {
...
WsEventMgr.prototype.emit = function (name, data) {
  var i, registeredSockets, socket;
  registeredSockets = this.clientRegistrations[name];
...
WsEventMgr.prototype.hasClientRegistration = function (name) {
  return this.clientRegistrations[name] && this.clientRegistrations[name].length;
};
